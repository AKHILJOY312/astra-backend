...
export class RequestEmailChangeUseCase implements IRequestEmailChangeUseCase {
  ...
  async execute(userId: string, newEmail: string) {
    ...
    const existing = await this._userRepo.findByEmail(newEmail);
    ...
    const recent = await this._otpRepo.countRecentByUserId(userId, oneHourAgo);

    if (recent >= 3) {
      throw new TooManyRequestError("Too many Request. Try again in 1 hour.");
    }
    const otp = this._authSvc.generateOtp(6);
    const otpHash = await this._authSvc.hashPassword(otp.toString());
    ...
    await this._otpRepo.upsert(emailChangeOtp);
    //Send Otp
    await this._emailSvc.sendEmailChangeOtp(newEmail, otp.toString());
